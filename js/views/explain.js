function renderSystemGauge(canvasId, value, colorLow, colorHigh) {
    ...
    new Chart(ctx.getContext('2d'), {
        ...
        data: {
            ...
            datasets: [{ data: [value, 100 - value], backgroundColor: [primaryColor, 'rgba(255,255,255,0.05)'], borderWidth: 0, circumference: 180, rotation: 270 }]
        },
        options: {
    ...
    appEl.innerHTML = `
        ...
        <div class="doc-container" style="max-width:900px;margin:0 auto;padding-top:2rem;padding-bottom:5rem">
            ...
            <div style="display:flex;gap:1rem;flex-wrap:wrap">
                ...
                ${targetView ? `<button class="intel-action-btn" onclick="switchView('${targetView}')" style="display:flex;align-items:center;gap:8px;background:var(--accent);color:#000;font-weight:800"><span class="material-symbols-outlined" style="font-size:18px">open_in_new</span> OPEN VIEW</button>` : ''}
            </div>
        </div>
    `;
}
...
function renderViewDocPage({ hub, hubIcon, hubColor = 'var(--accent)', title, viewId, summary, components }) {
    ...
    appEl.innerHTML = `
        <div class="view-header" style="padding-bottom:1rem">
            <div style="display:flex;align-items:center;gap:6px;font-size:0.6rem;font-weight:700;letter-spacing:1px;color:var(--text-dim);margin-bottom:1rem">
                <span class="material-symbols-outlined" style="font-size:13px;cursor:pointer;color:var(--text-dim)" onclick="switchView('help')">home</span>
                <span>›</span>
        ...
        <div style="max-width:920px;display:flex;flex-direction:column;gap:1.5rem;padding-bottom:5rem">
            ${components.map((c, i) => {
                ...
                return `
                <div style="background:rgba(255,255,255,0.015);border:1px solid rgba(255,255,255,0.07);border-radius:12px;overflow:hidden;transition:border-color 0.2s" onmouseover="this.style.borderColor='rgba(255,255,255,0.15)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.07)'">
                    <div style="padding:1.1rem 1.4rem;background:rgba(255,255,255,0.02);border-bottom:1px solid rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:space-between;gap:1rem;flex-wrap:wrap">
                    ...
                    <div style="padding:1.4rem">
                        <p style="font-size:0.88rem;line-height:1.75;color:var(--text);margin-bottom:1.2rem">${c.description}</p>
