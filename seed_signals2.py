...
TYPE_MESSAGES = {
    "ML_ALPHA_PREDICTION": "ML alpha detected: predicted 24h return {alpha:.1f}pct. Driver: RSI_14 ({conf:.0f}pct confidence)",
    ...
    "MACD_BULLISH_CROSS":  "MACD bullish crossover confirmed on {ticker} with volume spike {vol:.1f} sigma above mean",
    "MACD_BEARISH_CROSS":  "MACD bearish crossover on {ticker} - momentum turning negative",
    "VOLUME_SPIKE":        "Volume spike {vol:.1f} sigma above 20-day mean on {ticker} - breakout candidate",
}
...
for (sig_type, ticker, category, direction, severity, entry_price, days_ago, status, exit_pct) in SIGNALS:
    ...
    message = TYPE_MESSAGES.get(sig_type, sig_type + " signal on " + ticker).format(**msg_kwargs)
    ...
    c.execute(
        ...
        (sig_type, ticker, message, severity, entry_price, ts, z_score, alpha, direction, category, btc_corr, sentiment, db_status, closed_at, exit_price_val, final_roi, USER_EMAIL)
    )
    inserted += 1

conn.commit()
conn.close()
print("Done. Inserted " + str(inserted) + " signals into " + DB_PATH)
